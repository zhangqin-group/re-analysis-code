...
a <- as.data.frame(tpm1)

y<-merge(tsne.rot,a,by="row.names")
data=data[order(data$Lyz1),]

ggplot(data, aes(x = tSNE_1, y = tSNE_2, color = Lyz1)) +
     geom_point() +  # 绘制点图
     scale_color_viridis(option = "D", begin = 0, end = 1, 
                         direction = -1,  # 反转颜色顺序
                         discrete = FALSE)+theme_classic()
			
data=data[order(data$Nod2),]	
		
ggplot(data, aes(x = tSNE_1, y = tSNE_2, color = Nod2)) +
     geom_point() +  # 绘制点图
     scale_color_viridis(option = "D", begin = 0, end = 1, 
                         direction = -1,  # 反转颜色顺序
                         discrete = FALSE)+theme_classic()						 
